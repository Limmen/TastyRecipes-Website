               ...
               $commentid = \Flight::request()->data[TASTY_EDIT_KEY];
                ...
                if(isset($newUsername) and isset($newComment) and isset($commentid))
                {
                 ...
                 if($cookies['username'] == $newUsername)
                   {
                 $sql = "UPDATE meatballcomments SET username = '$newUsername' WHERE comment_id = '$commentid'";
                 $result = mysql_query($sql);
                 $sql2 = "UPDATE meatballcomments SET comment = '$newComment' WHERE comment_id = '$commentid'";
                 $result2 = mysql_query($sql2);
                 ?>
                <meta http-equiv="refresh" content="0; url=http://localhost/sem3/meatballs" />
                <?php
                }
                   else
                   {
